...
public class TestHistorySnapshotStoreImpl {
  ...
  public void testDefaultSnapshot() {
    ...
    List<String> associatedIds = testSubject.execute(getIds, DatabaseReference.getMain()).getEntries();
    assertEquals(6, associatedIds.size());
    getIds.setEntityCollection(COUNTER_HISTORY);
    getIds.setQuery(QueryUtils.in(ID_FIELD, jobIds));
    associatedIds = testSubject.execute(getIds, DatabaseReference.getMain()).getEntries();
    assertEquals(6, associatedIds.size());

    //check that their tasks are there
    getIds.setEntityCollection(TASK_HISTORY);
    getIds.setQuery(QueryUtils.in("jobId", jobIds));
    List<String> taskIds = testSubject.execute(getIds, DatabaseReference.getMain()).getEntries();
    ...
    jobIds = testSubject.execute(getIds, DatabaseReference.getMain()).getEntries();
    ...
    List<JobProfile> runningJobs = testSubject.execute(
    ...
    getIds.setQuery(QueryUtils.in("jobId", jobIds));
    taskIds = testSubject.execute(getIds, DatabaseReference.getMain()).getEntries();
    ...
    List<String> finishedTaskIds =
      testSubject.execute(getFinishedTaskIds, DatabaseReference.getMain()).getEntries();
